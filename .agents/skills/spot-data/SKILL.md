...
**Everything the scripts cache or dump lands in `.cache/`, beside `scripts/`.** `overpass.cache_path(name)` returns the path and creates the directory on first use; use it rather than writing next to the script, which is how 34 json files once came to outnumber the 13 scripts. The directory is gitignored as a whole - `ele-cache-shared.json` alone is 7 MB and grows with every sweep - so nothing in it survives a fresh clone. That matters for `named_hits.json`, `mtb_evidence.json` and `located.json`: they are overwritten on every run, so copy a batch out before re-running if you still need it.
